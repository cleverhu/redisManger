	...
	for i := 1; i <= 2; i++ {
		...
		if err == nil {
			...
			for _, v := range d {
				...
				dbs.Rds.Set("String_"+r.Id, string(data), time.Duration(rand.Int63()))
				for key, value := range m {
					dbs.Rds.LPush("List_"+r.Id, key+":"+fmt.Sprintf("%v", value))
					dbs.Rds.SAdd("Set_"+r.Id, key+":"+key+":"+fmt.Sprintf("%v", value))
					dbs.Rds.HSet("HSet_"+r.Id, key, value)
